...
- 对于 $\left.\partial_t u\right|_{t=0} = 0$ ，将函数在初始点做 Taylor 展开：
  $$
...
![image-20251211192920320](./assets/T4-1.png)
...
取$\Delta t = \frac{1}{\sqrt{\lambda}} \left( \frac{1}{\Delta x^2} + \frac{1}{\Delta y^2} \right)^{-1/2}$、`Nx=Ny=100`并取不同的$\lambda$，得到结果如下：

```
**测试不同时间步长dt下的数值稳定性**

取lambda=3.0计算得到的时间步长 dt = 4.08248e-03
  -> 误差分析 (lambda=3.0):
     t=0.00: Max Error=0.0000e+00, MSE=0.0000e+00
     t=1.00: Max Error=5.0157e-04, MSE=6.1654e-08
     t=2.00: Max Error=1.4779e-03, MSE=5.3528e-07
计算稳定，结束时刻 t=5.00

取lambda=2.0计算得到的时间步长 dt = 5.00000e-03
  -> 误差分析 (lambda=2.0):
     t=0.00: Max Error=0.0000e+00, MSE=0.0000e+00
     t=1.00: Max Error=4.1944e-04, MSE=4.3115e-08
     t=2.00: Max Error=1.2375e-03, MSE=3.7529e-07
计算稳定，结束时刻 t=5.00

取lambda=1.01计算得到的时间步长 dt = 7.03598e-03
  -> 误差分析 (lambda=1.01):
     t=0.00: Max Error=0.0000e+00, MSE=0.0000e+00
     t=1.00: Max Error=1.7906e-04, MSE=7.8580e-09
     t=2.00: Max Error=5.3123e-04, MSE=6.9161e-08
计算稳定，结束时刻 t=5.00

取lambda=0.99计算得到的时间步长 dt = 7.10669e-03
  -> 误差分析 (lambda=0.99):
     t=0.00: Max Error=0.0000e+00, MSE=0.0000e+00
     t=1.00: Max Error=1.7904e-04, MSE=7.4045e-09
     t=1.84: Max Error=8.3026e+04, MSE=1.1241e+09
计算发散，失稳时刻 t=1.85

取lambda=0.8计算得到的时间步长 dt = 7.90569e-03
  -> 误差分析 (lambda=0.8):
     t=0.00: Max Error=0.0000e+00, MSE=0.0000e+00
     t=0.43: Max Error=8.7746e+04, MSE=7.3076e+08
     t=0.43: Max Error=8.7746e+04, MSE=7.3076e+08
计算发散，失稳时刻 t=0.43
```

可以看到，当$\lambda \ge 1$时，数值解是稳定的；而当$\lambda \lt 1$时，数值解发散。

##### 空间步长

固定`dt`，取不同的`Nx`、`Ny`，得到结果如下：

```
**测试不同空间网格划分下的数值稳定性**:

  -> 误差分析 (Nx=50, Ny=50, dt=0.001):
     t=0.00: Max Error=0.0000e+00, MSE=0.0000e+00
     t=1.00: Max Error=2.6327e-03, MSE=1.6721e-06
     t=2.00: Max Error=7.7789e-03, MSE=1.4598e-05
计算稳定，结束时刻 t=5.00

  -> 误差分析 (Nx=100, Ny=100, dt=0.001):
     t=0.00: Max Error=0.0000e+00, MSE=0.0000e+00
     t=1.00: Max Error=6.5333e-04, MSE=1.0461e-07
     t=2.00: Max Error=1.9278e-03, MSE=9.1084e-07
计算稳定，结束时刻 t=5.00

  -> 误差分析 (Nx=150, Ny=150, dt=0.001):
     t=0.00: Max Error=0.0000e+00, MSE=0.0000e+00
     t=1.00: Max Error=2.8498e-04, MSE=2.0045e-08
     t=2.00: Max Error=8.4072e-04, MSE=1.7445e-07
计算稳定，结束时刻 t=5.00
```

可以看到，当`dt`一定时，随着网格划分越来越精细，即`Nx`、`Ny`越来越大，数值解的误差越来越小。另外，还可以注意到，在同一个模拟中，随着时间t的推移，数值解误差越来越大，即数值解误差是随着时间逐渐累积的。
...
其中，`all_randoms` 存储了所有实验的随机数，每一行对应一次实验； `bin_indices` 计算每个随机数落在哪个区间；`bin_counts` 统计每次实验中每个区间的频数，行为实验次数，列为区间索引。返回两个结果：落在 $[0.6, 0.7]$ 区间的比例`proportions_06_07`和每次实验的 $\chi^2$ 统计量`chi_squared`。
...
![image-20251214102623296](./assets/T5-3.png)
